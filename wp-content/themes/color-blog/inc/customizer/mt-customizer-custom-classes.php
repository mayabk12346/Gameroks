<?php
/**
 * Color Blog include the Customizer custom classes of customizer fields.
 *
 * @package Mystery Themes
 * @subpackage Color Blog
 * @since 1.0.0
 */
add_action( 'customize_register', 'color_blog_register_custom_controls' );

if( ! function_exists( 'color_blog_register_custom_controls' ) ) :
    /**
     * Register Custom Controls
     * 
     * @since 1.0.0
    */
    function color_blog_register_custom_controls( $wp_customize ){
        
        if ( ! class_exists( 'Color_Blog_Control_Toggle' ) ){
			/**
			 * Toggle control (modified checkbox)
			 */
			class Color_Blog_Control_Toggle extends WP_Customize_Control{
		...
		} //Ends Color_Blog_Control_Toggle
		
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------*/		
		if ( ! class_exists( 'Color_Blog_Control_Radio_Image' ) ){
			
			/**
			 * Radio Image control (modified radio).
			*/
			class Color_Blog_Control_Radio_Image extends WP_Customize_Control{
...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------*/		
		if( ! class_exists( 'Color_Blog_Control_Repeater' ) ) {
    
			/**
			 * Repeater control
			*/
			class Color_Blog_Control_Repeater extends WP_Customize_Control{
				...
				public $color_blog_box_label = '';
		
				public $color_blog_box_add_control = '';
				...
				public function __construct( $manager, $id, $args = array(), $fields = array() ) {
					...
					$this->fields = $fields;
					$this->color_blog_box_label = $args['color_blog_box_label_text'] ;
					$this->color_blog_box_add_control = $args['color_blog_box_add_control_text'];
				...
				?>
					...
					<ul class="mt-repeater-field-control-wrap">
						<?php $this->color_blog_get_fields(); ?>
					</ul>
					...
					<button type="button" class="button mt-repeater-add-control-field"><?php echo esc_html( $this->color_blog_box_add_control ); ?></button>
			<?php
				}

				private function color_blog_get_fields(){
					$fields = $this->fields;
					$values = json_decode( $this->value() );
				...
				?>
						...
						<h3 class="mt-repeater-field-title"><?php echo esc_html( $this->color_blog_box_label ); ?></h3>						
						...
						?>
							...
							<?php 
								...
								switch ( $field['type'] ) {
									 ...
									 */
									case 'icon':
										$color_blog_font_awesome_icon_array = color_blog_font_awesome_icon_array();
										echo '<div class="mt-repeater-selected-icon"><i class="'.esc_attr( $new_value ).'"></i><span><i class="fa fa-angle-down"></i></span></div><ul class="mt-repeater-icon-list clearfix">';
										foreach ( $color_blog_font_awesome_icon_array as $color_blog_font_awesome_icon ) {
											$icon_class = $new_value == $color_blog_font_awesome_icon ? 'icon-active' : '';
											echo '<li class='.esc_attr( $icon_class ).'><i class="'.esc_attr( $color_blog_font_awesome_icon ).'"></i></li>';
										}
									 ...
									 */
									case 'social_icon':
										$color_blog_font_awesome_social_icon_array = color_blog_font_awesome_social_icon_array();
										...
										foreach ( $color_blog_font_awesome_social_icon_array as $color_blog_font_awesome_icon ) {
											$icon_class = $new_value == $color_blog_font_awesome_icon ? 'icon-active' : '';
											echo '<li class='.esc_attr( $icon_class ).'><i class="'.esc_attr( $color_blog_font_awesome_icon ).'"></i></li>';
										}
										echo '</ul><input data-default="'.esc_attr( $default ).'" type="hidden" value="'.esc_attr( $new_value ).'" data-name="'.esc_attr( $key ).'"/>';
									 ...
									 */
									case 'dropdown_pages':
										$show_option_none = esc_html__( '&mdash; Select a page &mdash;', 'color-blog' );
									...
									case 'upload':
										$image_class = "";
										$upload_btn_label = esc_html__( 'Select Image', 'color-blog' );
										$remove_btn_label = esc_html__( 'Remove', 'color-blog' );
										if( $new_value ){ 
											$image_class = ' hidden';
										}
										echo '<div class="mt-fields-wrap"><div class="attachment-media-view"><div class="placeholder'. esc_attr( $image_class ).'">';
										esc_html_e( 'No image selected', 'color-blog' );
					...
					?>
							<div class="mt-clearfix mt-repeater-footer">
								...
								<a class="mt-repeater-field-remove" href="#remove"><?php esc_html_e( 'Delete', 'color-blog' ) ?></a> |
								<a class="mt-repeater-field-close" href="#close"><?php esc_html_e( 'Close', 'color-blog' ) ?></a>
								</div>
    ...
    }// Ends Color_blog_register_custom_controls
endif;
...
if( class_exists( 'WP_Customize_Section' ) ) {
     ...
     * @access public
     */
    class Color_Blog_Section_Upsell extends WP_Customize_Section {
        ...
        protected function render_template() { ?>
            ...
            <li id="accordion-section-{{ data.id }}" class="color-blog-upsell-accordion-section control-section control-section-{{ data.type }} cannot-expand">
                <h3 class="accordion-section-title"><a href="{{ data.pro_url }}" target="_blank">{{ data.title }}</a> </h3>
            </li>
        <?php }
    }// end Color_Blog_Section_Upsell
