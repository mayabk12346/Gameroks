...
class Color_Blog_Welcome {
	...
	public static function hide_notices() {
		if ( isset( $_GET['color-blog-hide-notice'] ) && isset( $_GET['_color_blog_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_color_blog_notice_nonce'], 'color_blog_hide_notices_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'color-blog' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'Cheat in &#8217; huh?', 'color-blog' ) );
			}

			$hide_notice = sanitize_text_field( $_GET['color-blog-hide-notice'] );
			update_option( 'color_blog_admin_notice_' . $hide_notice, 1 );
		}
	}
...
?>
		<div id="mt-theme-message" class="updated color-blog-message notice is-dismissible">
		...
		<div class="wrap about-wrap">
			...
			<div class="changelog">
				<div class="under-the-hood two-col">
					...
					<div class="col">
						<h3> <?php echo esc_html( 'Translate', 'color-blog' ).' '.esc_html( $theme_name ); ?> </h3>
						<p><?php esc_html_e( 'Click below to translate this theme into your own language.', 'color-blog' ) ?></p>
						<p>
							<a href="<?php echo esc_url( 'https://translate.wordpress.org/projects/wp-themes/color-blog' ); ?>" class="button button-secondary" target="_blank"><?php echo esc_html( 'Translate', 'color-blog' ).' '.esc_html( $theme_name ); ?></a>
						</p>
					</div>
				</div>
			</div><!-- .changelog -->
