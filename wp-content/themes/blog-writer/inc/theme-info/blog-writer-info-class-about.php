 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'blog_writer_info_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class blog_writer_info_page {
		/**
		 * Define the version of the class.
		 * @var string $version The blog_writer_info_page class version.
		 */
		...
		/**
		 * The single instance of blog_writer_info_page
		 * @var blog_writer_info_page $instance The  blog_writer_info_page instance.
		 */
		private static $instance;

		/**
		 * The Main blog_writer_info_page instance.
		 * We make sure that only one instance of blog_writer_info_page exists in the memory at one time.
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof blog_writer_info_page ) ) {
				self::$instance = new blog_writer_info_page;
		...
		public function welcome_admin_notice() {
			...
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && !get_option( 'blog_writer_notice_welcome' ) && 'themes.php' == $pagenow ) {
					...
					echo '<div class="blog-writer-info-about updated notice is-dismissible">';
					echo '<a class="notice-dismiss" href="' . esc_url(wp_nonce_url(remove_query_arg(array('activated'), add_query_arg('blog-writer-hide-notice', 'welcome')), 'blog_writer_hide_notices_nonce', '_blog_writer_notice_nonce')) . '" style="z-index: 0;padding: 10px;text-decoration: none;" >';
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'blog-writer'). '</span>';
					echo '</a>';
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if ( ! empty( $this->tabs ) ) {
					...
					foreach ( $this->tabs as $tab_key => $tab_name ) {
						...
						echo '<a href="' . esc_url( admin_url( 'themes.php?page=blog-writer-welcome' ) ) . '&tab=' . esc_attr( $tab_key ) . '" class="nav-tab ' . ( $active_tab == $tab_key ? 'nav-tab-active' : '' ) . '" role="tab" data-toggle="tab">';
		...
		*/
		public function theme_review() {
			echo '<div id="blog-writer-support-content" class="feature-section two-col">';
		...
		public function getting_started() {
			...
			if ( ! empty( $this->config['getting_started'] ) ) {
				...
				if ( ! empty( $getting_started ) ) {
					...
					echo '<div id="blog-writer-getting-started" class="feature-section three-col">';
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					echo '<div id="blog-writer-free-pro" class="feature-section">';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="blog-writer-free-pro-button">';
						echo '<td></td>';
