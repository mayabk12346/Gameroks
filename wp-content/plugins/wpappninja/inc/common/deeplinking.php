...
function wpappninja_cache_friendly($s) {
	...
	if (get_wpappninja_option('agressive_anti_cache') == '1') {
		...
		// valid url
		if (filter_var($s, FILTER_VALIDATE_URL) && !preg_match('#add-to-cart=#', $s) && !preg_match('#_wpnonce=#', $s) && !preg_match('#wpmobileexternal=#', $s) && !preg_match('#wpappninja_v=#', $s) && !preg_match('#\.(pdf|png|jpg|jpeg|gif|bmp)#', $s)) {
	...
	if (wpappninja_is_pwa()) {
		...
		// valid url
		if (filter_var($s, FILTER_VALIDATE_URL) && !preg_match('#add-to-cart=#', $s) && !preg_match('#_wpnonce=#', $s) && !preg_match('#wpmobileexternal=#', $s) && !preg_match('#is_wppwa=#', $s) && !preg_match('#\.(pdf|png|jpg|jpeg|gif|bmp)#', $s)) {
	...
	if (get_wpappninja_option('cache_friendly') == '1' && !preg_match('#add-to-cart=#', $s) && !preg_match('#wpmobileexternal=#', $s) && !preg_match('#_wpnonce=#', $s) && !$preventCache && !preg_match('#\.(pdf|png|jpg|jpeg|gif|bmp)#', $s)) {
