...
function wpappninja_cron() {
	...
	$query = $wpdb->get_results($wpdb->prepare("SELECT `id`, `id_post`, `titre`, `message`, `image`, `lang`, `category` FROM {$wpdb->prefix}wpappninja_push WHERE `send_date` < %s AND sended = '0' ORDER BY `id` DESC", current_time('timestamp')));
