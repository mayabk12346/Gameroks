...
function wpmobile_retrieve_password($user_login) {
    ...
    $message .= "'>".__('Click this link', 'wpappninja')."</a>";
    $message .= "<br/><br/>";
...
function wpmobile_username_password( $user, $username, $password ) {
	...
	if( $username == "" || $password == "" ) {
		wp_redirect(wpmobile_weglot(home_url() . '/?wpapp_shortcode=wpapp_login&login=fail') );
		exit;
	}
	}
}
...
function wpmobile_login_fail( $username ) {
	...
	if (is_wpappninja()) {
    	 ...
    	 if ( !empty($referrer) && !strstr($referrer,'wp-login') && !strstr($referrer,'wp-admin') ) {
        	  wp_redirect(wpmobile_weglot(home_url() . '/?wpapp_shortcode=wpapp_login&login=fail' ));  // let's append some information (login=failed) to the URL for the theme to use
          	exit;
     	}
     }
}
    ...
    if (!is_user_logged_in()) {
    	...
    	<form name="registerform" id="registerform" action="" method="post">
			...
			<p class="login-username">
				...
				<input style="width: 100%;padding: 10px;background: #fff;border: 1px solid #eee;" type="email" name="uemail" id="wpmobileresetmail" class="input input-with-value" value="'.$__email.'" size="20">
			</p>
...
function wpapp_history() {
	...
	if ($number_push == 0) {
		...
		'.__('No informations for now.', 'wpappninja').'</center>';
	}
...
function wpappninja_show_card($content_post) {
	...
	<div class="card wpappninja-card-header-pic wpappninja-card post" id="post-<?php echo $content_post->ID; ?>">
