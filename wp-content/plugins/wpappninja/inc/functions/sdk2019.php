...
function wpmobile_ajax_get_push() {
	...
	jQuery(function() {
		if (jQuery(window.location.hash).length) {
			jQuery('.page-content').animate({scrollTop: jQuery(window.location.hash).offset().top}, 300);
	...
	jQuery(window).on('hashchange', function () {
		if (jQuery(window.location.hash).length) {
			jQuery('.page-content').animate({scrollTop: jQuery(window.location.hash).position().top}, 300);
			window.location.hash = "";
			setTimeout(function() {
	...
	function wpmobile_ajax_get_push() {
		...
		jQuery.ajax({
			...
			success: function (response) {
	    		...
	    		if (json.length > 1) {
	    			app.dialog.confirm('<?php _e('You have unread notifications', 'wpappninja');?>', json.length + ' notifications', function(){document.location = '?wpapp_shortcode=wpapp_history';});
	...
	jQuery(function(){
		...
		setInterval(wpmobile_ajax_get_push, 120000);
...
function wpmobile_redirect_homepage() {
	...
	if (isset($_GET['wpmobile_homepage'])) {
		...
		if (is_user_logged_in()) {
	        if (get_wpappninja_option('login_redirect_after') != '') {
               	$home = get_wpappninja_option('login_redirect_after');
       		}
		}
		...
		<html <?php if (wpappninja_isIOS()) {echo ' manifest="' . WPAPPNINJA_ASSETS_3RD_URL . 'manifestcache.php" ';} ?> class="wpmobiledonthidesplash wpappninja" style="background:url(<?php echo $app_data['splashscreen'];?>) center center no-repeat;background-size:cover"><head><script>document.location = '<?php echo $home;?>';</script></head><body></body></html>
		<?php
		exit();
	}
}
...
html #root .summary.entry-summary {
	...
	html #root span.activity-read-more {
    	display: block;
	...
	jQuery(function() {
		...
		jQuery('#members-dir-list').bind("DOMSubtreeModified",function() {jQuery('.page-content').animate({scrollTop: 0}, 300);});
		jQuery('.activity-read-more a').attr('href', '');
