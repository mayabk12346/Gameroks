					...
					<td style="padding:0">
						...
						if (get_wpappninja_option('speed') != '1') {
							...
							echo '<div class="wpappninja_menuitem" id="wpappninja_item_'.$current_locale.''.$i.'" style="display:none;background:white;padding-top:22px;margin-bottom:0">';
								...
								// icon
								echo '<div ';if ($item['id'] == 'http://separatorend'){echo 'style="display:none;" ';}echo ' class="label_iconic" id="label_iconic_'.$i.'">';
								...
								foreach($files as $file) {
									if (get_wpappninja_option('speed') == '1') {
										...
										$file = preg_replace('#.*\/([a-z_\-]+)\.svg$#', '$1', $file);
										echo '<label onclick="jQuery(\'#label_iconic_'.$i.' input\').attr(\'checked\', false);jQuery(\'input\', this).attr(\'checked\', true);jQuery(\'#wpappninja_item_'.$current_locale.''.$i.'_icon\').attr(\'src\',\'' . WPAPPNINJA_SVG_URL . $file.'.svg\');"><input style="display:none" type="radio" name="' . WPAPPNINJA_SLUG . '[menu_reload_'.$current_locale.']['.$i.'][icon]" value="'.$file.'" ';if ($item['icon'] == $file){echo 'checked';}echo ' /><img src="' . WPAPPNINJA_SVG_URL . $file.'.svg" /><br/>'.preg_replace('#.svg#', '', preg_replace('#_#', ' ', ucfirst($file))).'</label>';
								...
								// edit icon link
								echo '<div style="';if ($item['id'] == 'http://separatorend'){echo 'display:none;';}echo 'float: left;margin-left: 100px;margin-top: -29px;"><a href="#" onclick="jQuery(\'#label_iconic_'.$i.' label\').css(\'display\', \'inline-block\');return false">'.__('Edit').'</a></div>';
...
<div style="clear:both"></div>
			...
			<input name="<?php echo WPAPPNINJA_SLUG;?>[redirection_type]" value="<?php echo get_wpappninja_option('redirection_type', '0'); ?>" type="hidden" />
