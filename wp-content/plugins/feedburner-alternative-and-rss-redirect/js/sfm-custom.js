...
SFM( document ).ready(function( $ )
{
  ...
  SFM("body").on("click", "#mainRssconnect", function(){
    ...
    var feedid  = SFM(this).parents("form").find("input[name='feedid']").val();
    var error   = false;
...
function ProcessAfterCustomActive(res)
{
    ...
    SFM('#custom_'+res.rid).parent().parent().parent().next('.sfrd_green_box1').find('#mainRssconnect').parents("form#calimingOptimizationForm").find("input[name='feed_id']").val(res_data.feed_id);
